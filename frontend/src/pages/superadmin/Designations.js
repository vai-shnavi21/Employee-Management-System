...
export default function Designations() {
  ...
  const load = () => Promise.all([api.get('/designations'), api.get('/organizations')])
    .then(([d, o]) => { setDesigs(d.data); setOrgs(o.data); });
  useEffect(() => { load(); }, []);
      ...
      <div className="card">
        <div className="card-body p-0">
          <table className="table table-hover mb-0">
            ...
            <tbody>
              {desigs.length === 0 && <tr><td colSpan={5} className="text-center text-muted py-4">No designations yet</td></tr>}
              {desigs.map((d, i) => (
                <tr key={d._id}>
